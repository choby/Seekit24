...
export default abstract class AnimationBase<T extends AnimatedLottieViewProps = AnimatedLottieViewProps> extends React.Component<T> {
    protected lottieView = React.createRef<LottieView>();
    public play(startFrame?: number, endFrame?: number) {
        this.lottieView.current?.play(startFrame, endFrame);
    }
    public reset() {
    ...
    public resume() {
        this.lottieView.current?.resume();
    }
}