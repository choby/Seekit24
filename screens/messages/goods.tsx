...
export default () => {
    ...
    useNetworkEffect(() => {
        refreshMessage().finally(() => setInitializing(false));
    }, true);

    const refreshMessage = async (reload?: boolean) => {
        if (reload !== true && messageList.isLastPage) return;
        const data = await getMessageList(EnumMsgType.REPLY, reload ? 1 : messageList.nextPage);
        if (data) {
            setMessageList({
                ...data,
    ...
    const onMessagePress = (item: Message) => {
        setMsgRead(item);
        navigation.navigate(Pages.GoodsDetail, { id: item.objectKey });
    };
        ...
        <PreLoader.View loading={initializing}>
            <PlaceHolder.NoMessageView isEmpty={messageList.total === 0} canRefresh={true} onRefresh={() => refreshMessage(true)}>
                <Loading.FlatList<Message>
                    ...
                    keyExtractor={item => `goods_${item.msgId}`}
